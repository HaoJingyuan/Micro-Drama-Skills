...
def generate_scene_reference(variant_info, output_path, model="gemini_3_pro_image"):
    ...
    print(f"Generating {Path(output_path).name}...")
    image_url = generate_image(model, prompt, "16:9")
...
print(f"Model: {model}\n")
stats = {"characters": 0, "scenes": 0, "props": 0, "failed": 0}
...
print("\n" + "=" * 60)
print("GENERATING SCENE REFERENCES")
print("=" * 60)
for scene in elements_config.get("scenes", []):
    print(f"\nScene: {scene['name']}")
    for variant in scene["variants"]:
        ...
        if generate_scene_reference(variant, output_path, model):
            stats["scenes"] += 1
        else:
            stats["failed"] += 1
        time.sleep(1)
...
print("\n" + "=" * 60)
print("GENERATING PROP REFERENCES")
print("=" * 60)
for prop in elements_config.get("props", []):
    print(f"\nProp: {prop['name']}")
    for variant in prop["variants"]:
        ...
        if generate_prop_reference(variant, output_path, model):
            stats["props"] += 1
        else:
            stats["failed"] += 1
        time.sleep(1)
