...
**术语说明**：
- **Asset（资产）**：人物、场景、道具及其变体的总称
- **变体（Variant）**：同一asset在不同情节中的视觉变化版本

#### 1.2 识别变体（重要原则）

**核心原则：按需创建，避免过度设计**

并不是每个asset都需要变体，也不是每个道具都需要参考图。只有在以下情况下才创建变体或参考图：

**何时需要变体：**
- ✅ **视觉特征发生重大变化**：如人物换装、场景时间变化、道具损坏等
- ✅ **剧情明确要求**：故事中明确描述了不同状态
- ❌ **不需要变体**：视觉特征基本一致，仅有微小差异

**何时需要参考图：**
- ✅ **关键角色**：主角、重要配角（必须生成）
- ✅ **主要场景**：故事发生的核心场景（必须生成）
- ✅ **关键道具**：对剧情有重要影响的道具（如铁证、信物、武器等）
- ❌ **次要道具**：普通物品（如杯子、纸张、笔等）不需要单独生成参考图

**示例说明：**

**需要变体的情况：**
- 人物：日常装 → 正式装 → 战斗装（服装明显不同）
- 场景：白天 → 夜晚（光线氛围完全不同）
- 道具：完整的蛋糕 → 碎裂的蛋糕（状态完全不同）

**不需要变体的情况：**
- 人物：仅表情不同（愤怒/高兴/悲伤）→ 不需要变体，分镜生成时通过prompt控制
- 场景：仅视角不同（正面/侧面）→ 不需要变体，分镜生成时通过镜头角度控制
- 道具：仅位置不同（桌上/手中）→ 不需要变体，分镜生成时通过构图控制
...
**人物变体**（根据剧情需要）：
- 不同服装（日常/正式/战斗等）- 仅当剧情中明确换装时创建
- 不同状态（健康/受伤）- 仅当视觉差异明显时创建
- 不同时期（年轻/成熟/老年）- 仅当跨越时间线时创建

**场景变体**（不同时间/氛围）：
- 不同时段（白天/夜晚/黄昏）- 仅当剧情跨越不同时段时创建
- 不同天气（晴天/雨天/雪天）- 仅当天气对剧情有影响时创建
- 不同状态（整洁/凌乱/损坏）- 仅当场景状态发生重大变化时创建

**道具变体**（不同状态）：
- 完整/损坏 - 仅当剧情中道具被破坏时创建
- 新旧程度 - 仅当时间跨度导致明显老化时创建
- 使用中/静置 - 通常不需要变体，通过分镜构图控制
